...
namespace BaseRepoCore
{
    public class SqlServerProvider : IProvider
    {
        ...
        public virtual string SelectQuery<T>(Expression<Func<T, bool>> expression, string tableName) where T : class
        {
        ...
        public virtual string SelectFirstQuery<T>(Expression<Func<T, bool>> expression, string tableName) where T : class
        {
            IEnumerable<string> columns = GetColumns(typeof(T)).Select(p => string.Format("[{0}].[{1}]", tableName, p));
