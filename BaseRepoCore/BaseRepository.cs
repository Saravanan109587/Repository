...
namespace BaseRepoCore
{
    ...
    public class Repository<T> : IRepository<T> where T : class 
    {
        ...
        public int Execute(string commandText, object parameters = null, int? timeout = null, IDbTransaction transaction = null)
        {
            using (IDataContext _dataContext = new DataContext(_connectionstring))
            {
    ...
    public class Repository 
    {
        ...
        public IEnumerable<T1> ExecuteProcedureSingleResult<T1>(string storedProcedureName, DynamicParameters parameters = null, int? timeout = null, IDbTransaction transaction = null) where T1 : class
        {
            using (IDataContext _dataContext = new DataContext(_connectionstring))
            {
                return _dataContext.ExecuteProcedureSingleResult<T1>(storedProcedureName, parameters, transaction);
